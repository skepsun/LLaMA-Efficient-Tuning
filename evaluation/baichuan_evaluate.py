...
def parse_argument():
    ...
    parser.add_argument(
        "--checkpoint_dir", type=str, default=None, help="model name or path"
    )
    parser.add_argument(
...
class CEval:
    ...
    #         prompt = f"<s><Human>以下是中国关于{self.TASK2DESC[task_name]}考试的单项选择题，请选出其中的正确答案。\n"
    #         # prompt = ""
    #         if shot != 0:
    #             shuffled = dataset["dev"].shuffle()
    #             for i in range(min(shot, len(shuffled))):
    #                 if i == 0:
    #                     prompt += self.build_example(shuffled[i], with_answer=True)[7:]
    #                 else:
    #                     prompt += self.build_example(shuffled[i], with_answer=True)
    #         prompt += self.build_example(data, with_answer=False)
